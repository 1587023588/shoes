...
import { getOrder, mapStatusToDisplay } from '../../../services/order/localOrder';
...
Page({
  ...
  getDetail() {
    ...
    // 本地订单优先
    const local = getOrder(this.orderNo);
    if (local) {
      const display = mapStatusToDisplay(local.status);
      const order = {
        orderId: local.orderNo,
        orderNo: local.orderNo,
        parentOrderNo: local.orderNo,
        storeId: '1000',
        storeName: '布鞋工坊直营店',
        orderStatus: local.status,
        orderStatusName: display.text,
        goodsAmountApp: String(local.totalAmount),
        paymentAmount: String(local.totalAmount),
        logisticsVO: { logisticsNo: '' },
        orderItemVOs: (local.items || []).map((g, idx) => ({
          id: `${local.orderNo}-${idx}`,
          goodsPictureUrl: g.primaryImage,
          goodsName: g.title,
          skuId: g.skuId,
          spuId: g.spuId,
          specifications: (g.specInfo || []).map((s) => ({ specTitle: s.specTitle, specValue: s.specValue })),
          tagPrice: null,
          actualPrice: String(g.price),
          buyQuantity: g.quantity,
          tagText: null,
          buttonVOs: [],
        })),
        buttonVOs: [],
      ...
      this.setData({
        order,
        _order,
        formatCreateTime: formatTime(order.createTime, 'YYYY-MM-DD HH:mm'),
        countDownTime: null,
        addressEditable: false,
        isPaid: true,
        invoiceStatus: 3,
    ...
    // 不再请求远程 mock，若不是本地订单已在上方返回
    Toast({
      context: this,
      selector: '#t-toast',
      message: '未找到该订单（可能已被清理）',
    });
    setTimeout(() => {
