...
import { createOrder } from '../../../services/order/localOrder';
import { clearCart, getCartItems } from '../../../services/cart/localCart';
...
Page({
  ...
  onSubmitLocalOrder() {
    ...
    const address = this.data.userAddress || null;
    const items = this.goodsRequestList || [];
    if (!items.length) {
      Toast({ context: this, selector: '#t-toast', message: '无商品', icon: '' });
      return;
    }
    this.setData({ creating: true });
    try {
      const order = createOrder({ items: items.map(i => ({
        spuId: i.spuId,
        skuId: i.skuId,
        title: i.goodsName || i.title,
        primaryImage: i.primaryImage || i.thumb,
        quantity: i.quantity || i.num || 1,
        price: i.price ? Number(i.price) : 0,
        specInfo: i.specInfo || (i.skuSpecLst ? i.skuSpecLst.map(s=>({specTitle:s.specTitle,specValue:s.specValue})) : []),
      })), address, remark: '' });
      // 清空购物车（只清理已下单的）
      const cart = getCartItems();
      const remain = cart.filter(c => !items.find(i => i.skuId === c.skuId && i.spuId === c.spuId));
      if (remain.length !== cart.length) {
        // 粗暴：若有差异则直接保存剩余
        wx.setStorageSync('cart.items', JSON.stringify(remain));
      }
      Toast({ context: this, selector: '#t-toast', message: '订单已生成', icon: 'success' });
      setTimeout(()=>{
        wx.redirectTo({ url: `/pages/order/order-detail/index?orderNo=${order.orderNo}` });
      }, 600);
    } catch(e) {
      Toast({ context: this, selector: '#t-toast', message: '创建失败', icon: '' });
